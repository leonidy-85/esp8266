...
UBUNTU_TRANSLATION_DOMAIN="led.leonid"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/led/tests/autopilot/run
aptest.depends  = sub-led sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/led/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-led sub-backend-App
